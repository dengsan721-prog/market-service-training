...
const salonInterviewStandardFile = path.join(repoRoot, 'data', 'salon-interview-standards.json');
const salonInterviewStandards = fs.existsSync(salonInterviewStandardFile)
  ? JSON.parse(fs.readFileSync(salonInterviewStandardFile, 'utf8'))
  : { title: '幸福沙龙采访', modules: [] };
...
const data = applyMarkdownOverrides({ modules, scriptLibrary, sources, rawMirrorSources, quickFlows, serviceFramework, directorThinking, salonReview, camp7Review, flowGuide, masterFlow, modelCollection, toolbox, interactiveQuestionLibraries, salonScriptTemplates, studentGrowthSteps, salonInterviewStandards });
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .salon-step-card:nth-child(3) { --step-accent: #ff9500; --step-tint: rgba(255,149,0,.12); }
    .salon-step-card:nth-child(4) { --step-accent: #af52de; --step-tint: rgba(175,82,222,.10); }
    .salon-step-card:nth-child(5) { --step-accent: #ff3b30; --step-tint: rgba(255,59,48,.09); }
    .salon-step-card:nth-child(6) { --step-accent: #00a7c7; --step-tint: rgba(0,167,199,.10); }
    .salon-step-card:nth-child(7) { --step-accent: #5856d6; --step-tint: rgba(88,86,214,.10); }
    .salon-step-card:nth-child(8) { --step-accent: #8e8e93; --step-tint: rgba(142,142,147,.12); }
    .content-panel .framework-grid,
    .content-panel .collection-grid,
    .content-panel .tool-grid {
    ...
    .mirror-standard-card.tool-card {
      display: block;
      padding: 0;
      background: #fff;
    }
    .mirror-standard-card .tool-card-body {
...
<body>
      ...
      <div class="content">
        ...
        <section class="band content-library workspace-panel" id="mirrorLibrary" hidden>
          <div class="band-inner">
            ...
            <div class="content-tabs" id="contentTabs" role="tablist" aria-label="内容库切换">
              ...
              <button type="button" data-content-target="salonInterviewStandards">幸福沙龙采访</button>
        ...
        <section class="band ability-module-card ability-camp7-review content-panel" id="abilityCamp7Review" hidden>
          <div class="band-inner">
            ...
            <div>
              <h4>关键问题</h4>
              <div class="salon-script-body mirror-card-list" id="camp7ReviewList"></div>
            </div>
          </div>
        </section>
        ...
        <section class="band ability-module-card ability-other content-panel" id="abilityFramework" hidden>
          <div class="band-inner">
            <div class="copy-title-row"><h2>市场服务</h2></div>
        ...
        <section class="band ability-module-card ability-other content-panel" id="abilityThinking" hidden>
          <div class="band-inner">
            <div class="copy-title-row"><h2>总监思维</h2></div>
        ...
        <section class="band ability-module-card ability-other content-panel" id="abilityTools" hidden>
          <div class="band-inner">
            <div class="copy-title-row"><h2>工具模型</h2></div>
        ...
        <section class="band ability-module-card content-panel" id="salonInterviewStandards" hidden>
          <div class="band-inner">
            <div class="copy-title-row"><h2>幸福沙龙采访</h2></div>
            <p>按幸福沙龙榜样采访标准拆成独立模块，采访、点评、筛选、禁忌和心法都可以单独复制。</p>
            <div class="salon-script-body" id="salonInterviewList"></div>
          </div>
        </section>
        ...
        <section class="band ability-module-card ability-model content-panel" id="modelCollection" hidden>
          <div class="band-inner">
            ...
            <div class="copy-title-row"><h2>先激活情感，再按16问一次性采集完整。</h2></div>
  ...
  <script>
    ...
    async function copyPanelText(button) {
      const target = document.getElementById(button.dataset.copyPanel || '');
      if (!target) return;
    ...
    function renderFramework() {
      frameworkGrid.innerHTML = manual.serviceFramework.map((item) => (
        '<article class="salon-step-card mirror-standard-card framework-card" data-search-text="' + safeHtml([item.number, item.title, item.body].join(' ')) + '">' +
          '<div class="salon-step-head">' +
            '<span class="salon-step-index">' + safeHtml(item.number) + '</span>' +
            '<h4 class="salon-step-title">' + safeHtml(item.title) + '</h4>' +
            compactCopyButton([item.number + ' ' + item.title, item.body].join('\\n'), 'mini-copy mirror-card-copy') +
          '</div>' +
          '<div class="salon-step-content">' + renderSalonLineBlocks(buildSalonLineBlocks(splitDisplayLines(item.body))) + '</div>' +
        '</article>'
      )).join('');
      bindCompactCopy(frameworkGrid);
    }

    function renderThinking() {
      thinkingGrid.innerHTML = manual.directorThinking.map((item, index) => (
        '<article class="salon-step-card mirror-standard-card thinking-card" data-search-text="' + safeHtml([item.title, item.body].join(' ')) + '">' +
          '<div class="salon-step-head">' +
            '<span class="salon-step-index">' + (index + 1) + '</span>' +
            '<h4 class="salon-step-title">' + safeHtml(item.title) + '</h4>' +
            compactCopyButton([(index + 1) + ' ' + item.title, item.body].join('\\n'), 'mini-copy mirror-card-copy') +
          '</div>' +
          '<div class="salon-step-content">' + renderSalonLineBlocks(buildSalonLineBlocks(splitDisplayLines(item.body))) + '</div>' +
        '</article>'
      )).join('');
      bindCompactCopy(thinkingGrid);
    }
    ...
    function renderReviewTools() {
      ...
      bindCompactCopy(camp7ReviewList);
    }

    function renderModelCollection() {
      collectionGrid.innerHTML = manual.modelCollection.steps.map((step, index) => (
        '<article class="salon-step-card mirror-standard-card collection-card" data-search-text="' + safeHtml([step.title, step.body, step.purpose].join(' ')) + '">' +
          '<div class="salon-step-head">' +
            '<span class="salon-step-index">' + (index + 1) + '</span>' +
            '<h4 class="salon-step-title">' + safeHtml(step.title) + '</h4>' +
            compactCopyButton(['第' + (index + 1) + '步 ' + step.title, step.body, step.purpose].join('\\n'), 'mini-copy model-step-copy') +
          '</div>' +
          '<div class="salon-step-content">' +
            renderSalonLineBlocks(buildSalonLineBlocks(splitDisplayLines(step.body))) +
            (step.purpose ? '<p class="salon-line formula"><strong>核心目的：</strong>' + safeHtml(step.purpose) + '</p>' : '') +
          '</div>' +
        '</article>'
      )).join('');
    ...
    function renderToolbox() {
      toolGrid.innerHTML = manual.toolbox.map((tool) => (
        '<article class="salon-step-card mirror-standard-card tool-card" data-search-text="' + safeHtml([tool.title, tool.text].join(' ')) + '">' +
          '<div class="salon-step-head">' +
            '<span class="salon-step-index">图</span>' +
            '<h4 class="salon-step-title">' + safeHtml(tool.title) + '</h4>' +
            compactCopyButton([tool.title, tool.text].join('\\n'), 'mini-copy tool-copy') +
          '</div>' +
          '<div class="salon-step-content tool-card-body">' +
            renderSalonLineBlocks(buildSalonLineBlocks(splitDisplayLines(tool.text))) +
            '<a class="image-link" href="' + safeHtml(tool.image) + '" target="_blank" rel="noopener">查看原图</a>' +
            '<a class="tool-image-preview" href="' + safeHtml(tool.image) + '" target="_blank" rel="noopener"><img src="' + safeHtml(tool.image) + '" alt="' + safeHtml(tool.title) + '"></a>' +
          '</div>' +
        '</article>'
      )).join('');
    ...
    function renderMirrorRawPanels() {
      (manual.rawMirrorSources || []).forEach((source) => {
        ...
        target.innerHTML = blocks.map((block, index) => {
          ...
          const displayLines = getRawDisplayLines(source.id, displayTitle, block.lines.length ? block.lines : [block.title]);
    ...
    function renderSalonScriptTemplates(activeIndex = 0) {
      ...
      salonScriptPanel.innerHTML =
        '<article class="salon-script-card" data-search-text="' + safeHtml([current.name, current.caption, current.content].join(' ')) + '">' +
          '<div class="salon-script-body">' + renderSalonScriptSteps(current.content || '') + '</div>' +
        '</article>';
    ...
    function renderSalonInterviewStandards() {
      const target = document.getElementById('salonInterviewList');
      const modules = manual.salonInterviewStandards?.modules || [];
      if (!target) return;
      if (!modules.length) {
        target.innerHTML = '<p class="question-empty">暂无幸福沙龙采访内容</p>';
        return;
      }
      target.innerHTML = modules.map((module, index) => {
        const lineBlocks = buildSalonLineBlocks(module.lines || []);
        const copyText = [module.title, ...lineBlocks.map((block) => block.text)].join('\\n').trim();
        return '<section class="salon-step-card salon-interview-card">' +
          '<div class="salon-step-head">' +
            '<span class="salon-step-index">' + safeHtml(module.order || String(index + 1)) + '</span>' +
            '<h4 class="salon-step-title">' + safeHtml(module.title || ('内容模块 ' + (index + 1))) + '</h4>' +
            compactCopyButton(copyText, 'mini-copy salon-interview-copy') +
          '</div>' +
          '<div class="salon-step-content">' + renderSalonLineBlocks(lineBlocks) + '</div>' +
        '</section>';
      }).join('');
    ...
    function renderSalonScriptSteps(content) {
      ...
      return steps.map((step, index) => {
        const lineBlocks = buildSalonLineBlocks(step.lines);
        const copyText = [step.title, ...lineBlocks.map((block) => block.text)].join('\\n').trim();
        return '<section class="salon-step-card">' +
          '<div class="salon-step-head">' +
            '<span class="salon-step-index">' + (index + 1) + '</span>' +
            '<h4 class="salon-step-title">' + safeHtml(step.title) + '</h4>' +
            compactCopyButton(copyText, 'mini-copy salon-step-copy') +
          '</div>' +
          '<div class="salon-step-content">' + renderSalonLineBlocks(lineBlocks) + '</div>' +
        '</section>';
      }).join('');
    ...
    function renderSalonLineBlocks(blocks) {
      return '<div class="salon-line-list">' + blocks.map((block) => (
        '<p class="salon-line ' + safeHtml(block.type) + '">' + formatSalonLine(block.text) + '</p>'
      )).join('') + '</div>';
    }

    function buildSalonLineBlocks(lines) {
      const blocks = [];
      let current = null;
      lines.map((line) => String(line || '').trim()).filter(Boolean).forEach((line) => {
        const type = classifySalonLine(line);
        if (!current || !shouldJoinSalonLine(current, line, type)) {
          if (current) blocks.push(current);
          current = { type, text: line };
          return;
        }
        current.text = joinSalonText(current.text, line);
      });
      if (current) blocks.push(current);
      return blocks;
    }

    function classifySalonLine(line) {
      const text = String(line || '').trim();
      if (/^步骤\s*[0-9]+[：:]/.test(text)) return 'step';
      if (/^场景[一二三四五六七八0-9]+/.test(text)) return 'scene';
      if (/^（?案例\s*[0-9一二三四五六七八]/.test(text) || /案例\s*[0-9一二三四五六七八][：:]/.test(text)) return 'case';
      ...
      if (/^(邀约公式|讲发心|发心模版|互动游戏|心愿闭环|打开身体的开关|温馨提示|总结|今天教会我们|核心逻辑|核心原则|分享几个已经验证过的融入方法|把改变梳理成了|具体做法|整个方法论的核心底色|榜样采访不是|让榜样感受到|让现场人相信|点评不是评价对错|点评方向|适用|话术示范|抓细节|简化版|最低标准|优先推荐|不必要求|筛选一句话|只要有人有一点正向动作|榜样采访，就是)/.test(text)) return 'formula';
      if (/^(第一个开关|第二个开关|第三个开关|第四个开关)/.test(text)) return 'bullet';
      if (/^(XX 老师|xxx|换个语气|用生气的语气|生气的语气)/.test(text)) return 'label';
      return 'plain';
    }

    function shouldJoinSalonLine(current, line, type) {
      if (type !== 'plain') return false;
      if (['task', 'question', 'scene'].includes(current.type)) return false;
      return !/[。！？!?；;：:】）)”’》]$/.test(current.text);
    }

    function joinSalonText(left, right) {
      const spacer = /[A-Za-z0-9]$/.test(left) && /^[A-Za-z0-9]/.test(right) ? ' ' : '';
      return left + spacer + right;
    }

    function formatSalonLine(text) {
      const raw = safeHtml(text);
      return raw
        .replace(/^([^：:]{2,16}[：:])/, '<strong>$1</strong>')
        .replace(/(步骤\s*[0-9]+[：:])/, '<strong>$1</strong>')
        .replace(/(场景[一二三四五六七八0-9]+[^：:]*[：:])/, '<strong>$1</strong>')
        .replace(/(案例\s*[0-9一二三四五六七八][：:])/, '<strong>$1</strong>');
    }

    function splitSalonScriptSteps(content) {
      const lines = String(content || '').split('\\n').map((line) => line.trim()).filter(Boolean);
      const steps = [];
      const intro = [];
      let current = null;
      lines.forEach((line) => {
        const match = line.match(/^第([一二三四五六七八])\\s*[：:.．]\\s*(.+)$/);
        if (match) {
    ...
    function getRawDisplayLines(sourceId, displayTitle, lines) {
      if (sourceId === 'rawSalon' && String(displayTitle || '').includes('注意事项')) {
        return [
          '1. 如果对方没有回应，还是重新回到第一步',
    ...
    function simplifyRawBlockTitle(title, lines) {
      ...
      if (!text) return '';
      if (text.startsWith('💕')) return text.length > 22 ? text.slice(0, 20) + '…' : text;
    ...
    function buildRawMirrorBlocks(lines, sourceTitle, sourceId = '') {
      ...
      return blocks.filter((block) => (
        (block.title || block.lines.length) &&
        !isEmptyWarningBlock(block)
      ));
    }

    function isEmptyWarningBlock(block) {
      const title = String(block.title || '').trim();
      const meaningfulLines = (block.lines || [])
        .map((line) => String(line || '').trim())
        .filter((line) => line && line !== '⚠️');
      return title === '⚠️' && meaningfulLines.length === 0;
    }
    ...
    function isRawBlockHeading(line, sourceTitle = '', sourceId = '') {
      ...
      if (sourceId === 'rawSalon' && /^💕/.test(text)) return true;
