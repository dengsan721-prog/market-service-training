  ...
  <script>
    ...
    function renderSalonScriptSteps(content) {
      ...
      return steps.map((step, index) => {
        const copyText = [step.title, ...step.lines].join('\\n').trim();
        return '<section class="salon-step-card">' +
          ...
          '</div>' +
          '<div class="salon-step-content">' + formatDisplayText(step.lines.join('\\n')) + '</div>' +
        '</section>';
      }).join('');
    }
    ...
    function splitSalonScriptSteps(content) {
      ...
      if (current) steps.push(current);
      if (intro.length && steps.length) {
        steps[0].lines = intro.concat(steps[0].lines);
      }
      return steps;
    }
